...
export default function TaskList({ tasks, setTasks, fetchTasks, loading, error, setError }: TaskListProps) {
  ...
  const setupDropTarget = useCallback((status: TaskStatus, element: HTMLElement) => {
    if (!element) return;
  ...
  useEffect(() => {
    ...
    (['TODO', 'IN_PROGRESS', 'COMPLETED'] as TaskStatus[]).forEach(columnStatus => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-13rem)]">
        {(['TODO', 'IN_PROGRESS', 'COMPLETED'] as TaskStatus[]).map((columnStatus) => (
          <div
            key={columnStatus}
            ref={(el) => {
              if (el) {
          ...
          >
            <div className="flex items-center justify-between mb-4 sticky top-0 bg-gray-50 z-10 py-2">
              <h3 className="font-semibold text-gray-700">{columnStatus.replace('_', ' ')}</h3>
              <span className="text-sm text-gray-500">
                {tasks.filter((task) => task.status === columnStatus).length} tasks
              </span>
            </div>
